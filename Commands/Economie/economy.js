...
module.exports = {
  ...
  execute: async (interaction) => {
    ...
    if (!Data) {
      ...
      embed.addFields({
        name: "Créer",
        value: "Créer votre compte galactique",
      });
    } else {
      // Si l'utilisateur a un compte galactique
      embed.addFields({
        name: "Supprimer",
        value: "Supprimer votre compte galactique",
      });
    }
    ...
    const embed2 = new Discord.EmbedBuilder()
      ...
      .addFields({
        name: "Succès",
        value:
          "Votre compte galactique a bien été créé. Vous avez été crédité de 1CG (Crédit Galactique)",
      })
    ...
    const embed3 = new Discord.EmbedBuilder()
      ...
      .addFields({
        name: "Succès",
        value: "Votre compte galactique a bien été supprimé.",
      })
      .setFooter({ text: `Demandé par ${interaction.user.username}` })
      .setTimestamp();
    ...
    collector.on("collect", async (index) => {
      if (index.customId === "page1") {
        if (index.user.id !== interaction.user.id) {
          return index.reply({
            content: `La seule personne autorisée à utiliser ce bouton est ${interaction.user.tag}`,
          });
        }

        if (Data)
          return interaction.reply({
            content: "Vous avez déjà un compte galactique.",
            ephemeral: true,
          });
