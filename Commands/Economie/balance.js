...
module.exports = {
    ...
    execute: async (interaction) => {
        ...
        let Data = await economySchema.findOne({ Guild: interaction.guild.id, User: interaction.user.id }).exec()

        if(!Data) return await interaction.reply({content: `Vous devez avoir un compte galactique pour exécuter cette commande.`, ephemeral:true})
        ...
        const embed = new Discord.EmbedBuilder()
            ...
            .addFields({name: "Solde", value: `**Banque:** ${bank}\n**Porte-monnaie:** ${wallet}\n**Total:** ${total}`})
