...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        const pollEmbed = new Discord.EmbedBuilder()
            ...
            .setDescription(`Choisissez entre les 2 options. Vous avez ${time} secondes !`)
            .addFields(
        ...
        try {
            const pollChannel = await client.channels.cache.get('1093205669747900537');
            ...
            const collector = pollMessage.createReactionCollector({ filter, time: ms(time) });
            ...
            await interaction.reply({ content: 'Le sondage a été créé', ephemeral: true });
        } catch(error) {
            console.error(error);
