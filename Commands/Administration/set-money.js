...
module.exports = {
    ...
    execute: async (interaction) => {
        ...
        if (amount < 0) {
            return await interaction.reply({ content: "Veuillez spécifier une quantité positive ou nulle de crédits à définir.", ephemeral: true })
        }

        const targetUser = interaction.options.getUser('utilisateur')
        if (!targetUser) {
            return await interaction.reply({ content: "Veuillez spécifier un utilisateur valide à qui définir des crédits.", ephemeral: true })
        }

        let TargetData = await economySchema.findOne({ Guild: guild.id, User: targetUser.id })
        ...
        await economySchema.findOneAndUpdate({ Guild: guild.id, User: targetUser.id }, { $set: { Wallet: amount } })

        const embed = new Discord.EmbedBuilder()
            .setColor('#0099ff')
            .setTitle('Rapport de définition de crédits')
            .setDescription(`L'utilisateur ${targetUser.username} a été fixé pour nombre de crédits à ${amount} crédits par ${user.username}.`)
            .setTimestamp()

        const targetChannel = guild.channels.cache.get("1092462727638827168")

        interaction.reply({ content: `Vous avez défini le montant de crédits de ${targetUser.username} à ${amount}.`, ephemeral: true })
    }
}
