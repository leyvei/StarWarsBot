...
module.exports = {
    ...
    execute: async (interaction) => {
        ...
        if (!targetUser) {
            return await interaction.reply({ content: "Veuillez spécifier un utilisateur valide à qui retirer des crédits.", ephemeral: true })
        }
        ...
        if (TargetData.Wallet < amount) {
            return await interaction.reply({ content: "L'utilisateur ciblé n'a pas suffisamment de crédits dans son portefeuille.", ephemeral: true })
        }

        await economySchema.findOneAndUpdate({ Guild: guild.id, User: targetUser.id }, { $inc: { Wallet: -amount } })

        const embed = new Discord.EmbedBuilder()
            .setColor('#0099ff')
            .setTitle('Rapport de retrait de crédits')
            .setDescription(`L'utilisateur ${targetUser.username} a été retiré de ${amount} crédits par ${user.username}.`)
            .setTimestamp()

        const targetChannel = guild.channels.cache.get("1092462727638827168")

        await targetChannel.send({ embeds: [embed] })
        interaction.reply({ content: `Vous avez retiré ${amount} crédits à ${targetUser.username}.`, ephemeral: true })
    }
}