...
module.exports = {
    ...
    execute: async (interaction) => {
        ...
        if (amount <= 0) {
            return await interaction.reply({ content: "Veuillez spécifier une quantité positive de crédits à ajouter.", ephemeral: true })
        }

        const targetUser = interaction.options.getUser('user')
        if (!targetUser) {
            return await interaction.reply({ content: "Veuillez spécifier un utilisateur valide à qui ajouter des crédits.", ephemeral: true })
        }
        ...
        const embed = new Discord.EmbedBuilder()
            ...
            .setDescription(`L'utilisateur ${targetUser.username} a été ajouté de ${amount} crédits par ${user.username}.`)
            .setTimestamp()
        ...
        interaction.reply({ content: `Vous avez ajouté ${amount} crédits à ${targetUser.username}.`, ephemeral: true })
    }
}