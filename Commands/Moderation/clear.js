...
module.exports = {
  data: new Discord.SlashCommandBuilder()
    ...
    .addIntegerOption((option) =>
      option
        .setName("montant")
        .setDescription("Combien de messages souhaitez-vous supprimer")
        .setRequired(true)
        .setMinValue(1)
        .setMaxValue(99)
    )
    .addUserOption((option) =>
      option
        .setName("user")
        .setDescription(
          "Souhaitez-vous supprimer les messages d'un membre ? (Si oui, renseignez-le)"
        )
        .setRequired(false)
    ),
  ...
  execute: async (interaction, client) => {
    ...
    const filteredMessages = user
      ? messages.filter((m) => m.author.id === user)
      : messages;
    await channel.bulkDelete(filteredMessages, true);

    // Envoi d'un message de confirmation
    await interaction.reply({
      content: `**${filteredMessages.size}** messages ont été supprimés.`,
      ephemeral: true,
    });
  },
};
